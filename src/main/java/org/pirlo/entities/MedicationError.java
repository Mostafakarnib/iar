...
@NamedQueries(
        {
            @NamedQuery(name = MedicationError.FIND_ALL, query = "SELECT m FROM MedicationError m")
            ,@NamedQuery(name = MedicationError.FIND_BY_ID, query = "SELECT m FROM MedicationError m WHERE m.id = :id")
        })
public class MedicationError implements Serializable {

    private static final long serialVersionUID = 1L;

    public static final String FIND_ALL = "MedicationError.findAll";
    public static final String FIND_BY_ID = "MedicationError.findById";
